import { ActionRowBuilder, ButtonBuilder, ButtonStyle, type ChatInputCommandInteraction, ComponentType, EmbedBuilder, SlashCommandBuilder } from "discord.js";
import { MyEmbeds } from "../../lib/embeds.ts";
import { findPageName } from "../../lib/mediawiki.ts";
import { LiquidDB } from "../../lib/api.ts";
import type { SquadPlayer } from "../../lib/types.ts";
...
export const data = new SlashCommandBuilder()
	...
	.addSubcommand((subcommand) =>
		subcommand
			...
			.addStringOption((option) => option.setName("query").setDescription("Type your query here.").setRequired(true))
	)
	.addSubcommand((subcommand) =>
		subcommand
			.setName("map")
...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	const sendToChatButton = new ButtonBuilder().setCustomId("sendtochat").setLabel("Send to chat!").setStyle(ButtonStyle.Primary);
	const Row = new ActionRowBuilder<ButtonBuilder>().addComponents(sendToChatButton);
	...
	switch (interaction.options.getSubcommand()) {
		...
		case "map":
			...
			break;
		case "player":
			if ((await helper.checkAllow(interaction.user.id)) && (await helper.hasPun(query))) {
				sendEmbed = await e.makePun(await helper.checkPun(query), interaction.client);
				break;
			}
		...
		case "team":
			{
				...
				if (teamMem.length == 0) {
					...
					for (const member of teamMem) {
						...
						Row.addComponents(
							new ButtonBuilder().setCustomId(member.id.toLowerCase()).setLabel(member.id).setEmoji("<:bounty:1291683164758212668>").setStyle(ButtonStyle.Secondary)
