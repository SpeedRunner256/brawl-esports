...
export const data = new SlashCommandBuilder()
	...
	.addStringOption((option) => option.setName("pagename").setDescription("Page link for the match").setRequired(true));
...
export async function execute(interaction: ChatInputCommandInteraction) {
	...
	for (const match of matchArray) {
		if (match.match2opponents[0].name == "TBD" || match.match2opponents[1].name == "TBD") {
			continue; // match not played
		}
		const matchName = `${matchMenuCounter}. ${match.match2opponents[0].name} vs ${match.match2opponents[1].name}`;
		matchSelectMenuFields.push(new StringSelectMenuOptionBuilder().setLabel(matchName).setValue(`${matchMenuCounter - 1}`));
		matchMenuCounter += 1;
	}
	const selectMatch = new StringSelectMenuBuilder().setCustomId("matchselect").setPlaceholder("What match to show?").addOptions(matchSelectMenuFields);
	const matchSelectionRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(selectMatch);
	...
	const home = new ButtonBuilder().setCustomId("home").setLabel("Home").setStyle(ButtonStyle.Primary);
	const next = new ButtonBuilder().setCustomId("next").setLabel("Next").setStyle(ButtonStyle.Secondary);
	const navigationRow = new ActionRowBuilder<ButtonBuilder>().addComponents(previous, home, next);
	...
	navigationButtonCollector.on("collect", async (i) => {
		...
		if (matchArray[matchNumber].match2games.length == 0) {
			await i.reply({
				content: "Games are NOT logged for this set, the buttons do nothing at all.",
				ephemeral: true,
			});
			return;
		}
		...
		if (matchArray[matchNumber].match2games[gameNumber].resulttype == "np") {
			gameNumber = oldGameNumber;
			await i.reply({
				content: "Navigation button warning -> the match asked does not exist.",
				ephemeral: true,
			});
			return;
		}
