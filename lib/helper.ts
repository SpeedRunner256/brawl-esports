...
import { ExtraData, Prediction, PunData, SquadPlayer, Team } from "./types.ts";
...
export class Helper {
	...
	private staffFmt(players: SquadPlayer[]): string {
		return players
			.map(
				(player) =>
					`**[${player.id}](https://liquipedia.net/brawlstars/${new URLSearchParams(player.link).toString()})**: ${player.role} - ${this.formatDate(player.joindate)}`
			)
			.join("\n");
	}
	...
	async getGamemodeBans(name: string) {
		const unsorteddata = JSON.parse(await readFile("db/matches.json", "utf8")) as Match[];
		...
		for (const match of data) {
			for (const game of match.match2games) {
				...
				for (const brawler of Object.values(game.participants)) {
					...
					if (Object.values(brawler)[0].toLowerCase() == name.toLowerCase()) {
						answer.push(match);
						if (answer.length == 20) {
							return answer;
						}
	...
	async sortByPlayer(name: string) {
		const udata = JSON.parse(await readFile("db/matches.json", "utf8")) as Match[];
...
export class Config {
	...
	async userAlreadyVoted(predictionNumber: string, ID: string): Promise<boolean> {
		const data = JSON.parse(await readFile("db/prediction.json", "utf8"));
		const prediction = data[predictionNumber];
		return prediction.userChoices[1].includes(ID) || prediction.userChoices[2].includes(ID);
	}
	...
	async log(logMessage: string, logContent: string, interaction: ChatInputCommandInteraction): Promise<void> {
		...
		(interaction.client.channels.cache.get(log) as TextChannel).send(`${logMessage}\`\`\`json\n${logContent}\`\`\``);
	}
}
