import type { Player, Brawler, GameMode, Groups, Map, Match, Match2Games, Match2Opponents, Match2Players, gamePlayer, SquadPlayer, Team, Club, BattleLog } from "./types.ts";
import {
...
type Result = Player | Team | Match[] | Groups[] | SquadPlayer[] | Brawler | GameMode | Map | undefined;
export class LiquidDB {
	 ...
	 */
	static async get(type: "brawler" | "gamemode" | "map" | "player" | "team" | "match" | "teammember" | "group", query: string) {
							...
							starPowers: [
								{
									...
									description: brawler.starPowers[0].description,
								},
								{
									name: brawler.starPowers[1].name,
									description: brawler.starPowers[1].description,
								},
							],
							gadgets: [
								{
	...
	private static async gameMode(query: string): Promise<GameMode | undefined> {
			...
			.then((data) => {
				for (const mode of data.list) {
					if (query.toLowerCase() == mode.name.toLowerCase()) {
						const { name, title, color, description, shortDescription, link, imageUrl } = mode;
						return {
							name,
							title,
							color,
	...
	private static async player(name: string): Promise<Player | undefined> {
		...
		const Player = await fetch(`https://api.liquipedia.net/api/v3/player?${params}`, { headers })
			.then((response) => response.json())
			.then((data) => {
				const { pagename, id, nationality, region, teampagename, links, status, earnings } = data.result[0];
				if (!pagename) {
	...
	private static async team(name: string): Promise<Team | undefined> {
		...
		const { headers, params } = LiquidDB.queryHeadersParams(name);
		const Team = await fetch(`https://api.liquipedia.net/api/v3/team?${params}`, { headers })
			.then((response) => response.json())
			.then((data) => {
				// Get team
				const { pagename, name, region, logodarkurl, textlesslogodarkurl, status, createdate, disbanddate, links } = data.result[0];
		...
		await fetch(`https://api.liquipedia.net/api/v3/squadplayer?${param.toString()}`, { headers: headers })
			.then((response) => response.json())
			.then((data) => {
				for (const memberNumber in Object.values(data.result)) {
					const { id, role, link, type, status, joindate, nationality } = data.result[memberNumber];
					Team.players.push({
	...
	private static async match(name: string): Promise<Match[] | undefined> {
		...
		for (const match of matches) {
			if (match.pagename == name) {
				...
				if (match.match2games[0].scores.length == 0 && match.match2games[0].resulttype != "np") {
					break;
				}
				return matches;
			}
		...
		const { headers, params } = LiquidDB.queryHeadersParams(name);
		const Match = await fetch(`https://api.liquipedia.net/api/v3/match?${params}`, { headers })
			.then((response) => response.json())
			.then((data) => {
				const answer: Match[] = [];
				if (!data.result) {
				...
				for (const result of data.result) {
					const { pagename, objectname, winner, finished, stream, tickername, icondarkurl, liquipediatiertype } = result;
					...
					for (const match of result.match2games) {
						const { map, scores, participants, winner, date, extradata, resulttype } = match;
						match2games.push({
							map,
							scores,
							winner,
	...
	private static async group(name: string): Promise<Groups[] | undefined> {
		const { headers, params } = LiquidDB.queryHeadersParams(name);
		const Group = (await fetch(`https://api.liquipedia.net/api/v3/standingsentry?${[params]}`, { headers })
			.then((response) => response.json())
			.then((data) => {
				data = data["result"];
				const answer: Groups[] = [];
				...
				for (const group of data) {
					const { pagename, standingsindex, opponentname, placement, scoreboard } = group;
					answer.push({
						pagename,
						standingsindex,
						opponentname,
	...
	private static async teammember(name: string): Promise<SquadPlayer[] | undefined> {
...
const apikey = process.env.BS_TOKEN;
if (!apikey) {
	throw new Error("no api key");
}
type BAPIType = "player" | "club";
export class BrawlApiWrapper {
	private headers = {
	...
	constructor({ type, playerHash, clubHash }: { type: BAPIType; playerHash?: string; clubHash?: string }) {
		this.type = type;
		this.playerTag = playerHash?.replace(/^#/g, "");
		this.clubTag = clubHash?.replace(/^#/g, "");
	}
	async player(): Promise<gamePlayer> {
		if (!this.playerTag) {
			throw new Error("A player tag was not provided.");
		}
		const result = (await fetch(`https://bsproxy.royaleapi.dev/v1/players/%23${this.playerTag}`, {
		...
		result.image = `https://brawlbot.xyz/api/image/rank/${this.playerTag.replace(/^#/, "")}`;
		return result;
	}
	async battleLog(): Promise<BattleLog> {
		if (!this.playerTag) {
			throw new Error("A player tag was not provided.");
		}
		const result = (await fetch(`https://bsproxy.royaleapi.dev/v1/players/%23${this.playerTag}/battlelog`, {
			headers: this.headers,
		}).then((response) => {
			if (response.status != 200) {
				throw new InvalidPlayerTagError();
	...
	async club() {
		if (!this.clubTag) {
			throw new Error("A club tag was not provided.");
		}
		const result = (await fetch(`https://bsproxy.royaleapi.dev/v1/clubs/%23${this.clubTag}`, {
			headers: this.headers,
		}).then((response) => {
			if (response.status != 200) {
				throw new InvalidPlayerTagError();
	...
	async brawlerCount() {
		const brawlers = (await fetch("https://api.brawlify.com/v1/brawlers")
			.then((response) => response.json())
			.then((data) => data.list.length)) as number;
		return brawlers;
	}
	async gearCount(): Promise<{ superRareGears: number; mythicGears: number; epicGears: number }> {
		return {
			superRareGears: (await this.brawlerCount()) * 6,
			mythicGears: 10,
			epicGears: 25,
		};
	}
